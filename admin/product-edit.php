...
if(isset($_POST['form1'])) {
    ...
    if(empty($_POST['ecat_id'])) {
        $valid = 0;
        $error_message .= "Bạn phải chọn danh mục cấp cuối cùng<br>";
    }
    ...
    if($valid == 1) {
        ...
        if($path == '') {
         ...
         // Xóa ảnh hiện tại khỏi thư mục
        	unlink('../assets/uploads/'.$_POST['current_photo']);
         // Tạo tên file mới cho ảnh nổi bật
        ...
        if(isset($_POST['size'])) {
         // Xóa tất cả kích thước sản phẩm cũ liên kết với sản phẩm hiện tại
        	$statement = $pdo->prepare("DELETE FROM tbl_product_size WHERE p_id=?");
        	$statement->execute(array($_REQUEST['id']));
         // Thêm kích thước mới được chọn vào cơ sở dữ liệu
			foreach($_POST['size'] as $value) {
				$statement = $pdo->prepare("INSERT INTO tbl_product_size (size_id,p_id) VALUES (?,?)");
				$statement->execute(array($value,$_REQUEST['id']));
			}
		} else {
		// Nếu không có kích thước nào được chọn, xóa tất cả kích thước liên quan đến sản phẩm
			$statement = $pdo->prepare("DELETE FROM tbl_product_size WHERE p_id=?");
        	$statement->execute(array($_REQUEST['id']));
		}

		if(isset($_POST['color'])) {
		// Xóa tất cả màu sản phẩm cũ liên kết với sản phẩm hiện tại
			$statement = $pdo->prepare("DELETE FROM tbl_product_color WHERE p_id=?");
        	$statement->execute(array($_REQUEST['id']));
         // Thêm màu mới được chọn vào cơ sở dữ liệu
			foreach($_POST['color'] as $value) {
				$statement = $pdo->prepare("INSERT INTO tbl_product_color (color_id,p_id) VALUES (?,?)");
				$statement->execute(array($value,$_REQUEST['id']));
			}
		} else {
		// Nếu không có màu nào được chọn, xóa tất cả màu liên quan đến sản phẩm
			$statement = $pdo->prepare("DELETE FROM tbl_product_color WHERE p_id=?");
        	$statement->execute(array($_REQUEST['id']));
		}
				...
				<div class="box box-info">
					<div class="box-body">
						<div class="form-group">
							<label for="" class="col-sm-3 control-label">Tên Danh Mục Cao Cấp <span>*</span></label>
							<div class="col-sm-4">
								<select name="tcat_id" class="form-control select2 top-cat">
		                            <option value="">Chọn Danh Mục Cao Cấp</option>
		                            <?php
						...
						<div class="form-group">
							<label for="" class="col-sm-3 control-label">=Giá Cũ<br><span style="font-size:10px;font-weight:normal;">(In VNĐ)</span></label>
							<div class="col-sm-4">
						...
						<div class="form-group">
							<label for="" class="col-sm-3 control-label">Sản Phẩm Đang Hoạt Động Không?</label>
							<div class="col-sm-8">
