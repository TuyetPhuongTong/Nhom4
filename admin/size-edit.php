...
if(isset($_POST['form1'])) {
    ...
    if(empty($_POST['size_name'])) {
        $valid = 0;
        $error_message .= "Tên kích cỡ không được bỏ trống <br>";
    } else {
		// Kiểm tra trùng lặp
    	// Kiểm tra dữ liệu này đã có trong db chưa
    	...
    	if($total) {
    		$valid = 0;
        	$error_message .= 'Kích cỡ này đã tồn tại<br>';
    	}
    }
    ...
    if($valid == 1) {    	
    	...
    	$success_message = 'Kích cỡ được thêm thành công.';
    }
}
?>
