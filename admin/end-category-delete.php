...
<?php
	...
	for($i=0;$i<count($p_ids);$i++) {
		...
		foreach ($result as $row) {
			...
			unlink('../assets/uploads/'.$p_featured_photo); // Xóa ảnh đại diện
		}
		...
		foreach ($result as $row) {
			$photo = $row['photo'];
			unlink('../assets/uploads/product_photos/'.$photo); // Xóa ảnh sản phẩm
		}

		// Xóa trong tbl_product
		$statement = $pdo->prepare("DELETE FROM tbl_product WHERE p_id=?");
		$statement->execute(array($p_ids[$i]));

		// Xóa trong tbl_product_photo
		$statement = $pdo->prepare("DELETE FROM tbl_product_photo WHERE p_id=?");
		$statement->execute(array($p_ids[$i]));

		// Xóa trong tbl_product_size
		$statement = $pdo->prepare("DELETE FROM tbl_product_size WHERE p_id=?");
		$statement->execute(array($p_ids[$i]));

		// Xóa trong tbl_product_color
